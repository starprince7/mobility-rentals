import React, { ComponentProps, useRef, memo, forwardRef, useImperativeHandle } from "react"
...
function FancyInputComponent<T extends FieldValues = FieldValues>(
  ...
  // Styling classes
  const containerClassName = `border-2 ${
    error ? "border-red-500" : "border-zinc-200"
  } rounded-full px-8 py-2 justify-center items-start ${className}`
  const inputStyle = { fontFamily: "KumbhSansRegular" }
  const inputClassName = "pb-1.5 android:py-0 w-full placeholder:text-gray-300 ios:text-lg"
  const labelClassName = "bg-neutral-100 w-fit text-sm text-zinc-500 font-semibold"
    ...
    return (
      <Controller
        ...
        render={({
          ...
          <>
            <Pressable style={[style, { borderColor: error || fieldError ? "red" : "#E5E7EB" }]} onPress={handleInputPress} className={containerClassName}>
  ...
  return (
    <>
      <Pressable style={style} onPress={handleInputPress} className={containerClassName}>
        <FancyText className={labelClassName}>{label}</FancyText>
        <TextInput
          value={value}
      ...
      {error ? <FancyText className="text-xs text-red-500 ml-2">{error}</FancyText> : null}
    </>
  )
}
